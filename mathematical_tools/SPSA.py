...
class SPSAGradientCalculator:
    ...
    def calculate_gradient(self, qcs, H, w, num_qubits, parameters,order=1):
        ...
        This method performs a stochastic calculation of gradient in 1st and 2nd order for all parameters at a time, in each circuit (except for the last two qubits)
        ...
        :order: Order of SPSA gradient estimation (default = 1 or 2).
        ...
        for i in range(num_qubits - 2):
            spsa_vector_0 = np.random.choice([-1, 1], size=len(parameters)) # perturbation vector 0
            ...
            spsa_vector = delta*spsa_vector_0 # perturbation vector, quite simplified due to -1/+1 even-choice distribution. Can be modified for more complex distributions.
            ...
            if order==1:
                ...
                grad = w[i] * 1/delta * (self.expectation_calculator.calculate(qcs[i], parameters1, H) -
                                        self.expectation_calculator.calculate(qcs[i], parameters2, H)) * spsa_vector_0
                
                # Update gradients using pseudo-inverse of Fubini matrix approximation
                term = np.linalg.pinv(spsa_fubini_term).dot(grad)
                gradients += term
                cost += w[i] * self.expectation_calculator.calculate(qcs[i], parameters, H)
