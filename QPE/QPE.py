...
if __name__ == '__main__':
    ...
    # For demonstration, we define a Hamiltonian for H₂ using opflow.
    # (In this example the Hamiltonian is defined as: H = (0.33 Z⊗I) + (3.24 I⊗Z) + (1.17 Z⊗Z)
    # scaled by t/trotter_number.)
    trotter_number = 2
    t = 1
    H = (0.33*t/trotter_number*(Z ^ I)) + (3.24*t/trotter_number*(I ^ Z)) + (1.17*t/trotter_number*(Z ^ Z))
    # Obtain the evolution operator U = exp(iH) (using Qiskit's opflow)
    U_op = H.exp_i()
    U_matrix = U_op.to_matrix()
    
    # --- QPE parameters ---
    w_qubits = 3  # number of witness (ancilla) qubits
    s_qubits = 2  # number of system qubits
    # Define an initial state for the system (of dimension 2^(s_qubits) = 4)
    initial_state = np.array([1, 0, 0, 0])
    
    # --- Instantiate QPEAlgorithm ---
    qpe_algo = QPEAlgorithm(w_qubits, s_qubits, U_matrix,
                            trotter_number=trotter_number, t=t, initial_state=initial_state)
    
    # --- Build and draw the QPE circuit ---
    qpe_circ = qpe_algo.build_qpe_circuit()
    print(qpe_circ.draw('mpl'))
    
    # --- Run QPE on Qiskit ---
    counts = qpe_algo.run_qiskit(shots=8192)
    print("Qiskit counts:", counts)
    eigenvalues = QPEAlgorithm.plot_to_eigenval(counts, t=t, n=1)
    print("Estimated eigenvalue(s):", eigenvalues[0][1])  # for example, print the negative estimate
    
    # --- Convert to and run on QLM ---
    print("\nRunning on QLM:")
    qlm_result = qpe_algo.run_qlm(shots=100)
    for sample in qlm_result:
        print("State %s: probability %s +/- %s" % (sample.state, sample.probability, sample.err))
    
    # --- Convert to and run on Cirq ---
    print("\nRunning on Cirq:")
    cirq_result = qpe_algo.run_cirq(repetitions=1)
    print("Cirq result:")
    print(cirq_result)
